...
class PropertyVideoForm(forms.ModelForm):
    ...
    def clean_video(self):
        ...
        # Validate only if a new video is being uploaded
        if isinstance(video, UploadedFile):
            ...
            if video.content_type not in valid_mime_types:
                raise ValidationError('Unsupported file type. Please upload a valid video format.')

            if video.size > 200 * 1024 * 1024:  # 200MB
                raise ValidationError('Please keep the file size under 200MB.')
